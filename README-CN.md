...
- **自定义分类**: 通过设置菜单创建、重命名和删除你自己的分类。
- **持久化存储**: 分类设置基于应用路径保存，重启后您的工作区依然井井有条。

### ⚙️ 全方位定制
- **全局唤醒快捷键**: 使用自定义快捷键从任何地方唤醒应用（默认 `Alt+Space`）。支持更改为 `Cmd+P`, `Shift+Space` 等。
...
- **快速预览**: 在任何应用上按 `空格键` 即可即时预览其详细信息（路径、版本、使用次数）。
...
mac-app-control/
├── src/                    # React 前端
│   ├── App.tsx             # 应用逻辑、设置弹窗、过滤功能
│   ├── App.css             # 毛玻璃样式和主题变量
│   └── main.tsx            # React 入口文件
├── src-tauri/              # Rust 后端
│   ├── src/
│   │   └── lib.rs          # 系统命令、持久化、快捷键注册
│   ├── Cargo.toml          # Rust 依赖
